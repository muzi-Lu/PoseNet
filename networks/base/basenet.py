...
class BaseNet(nn.Module):
    ...
    def set_optimizer_(self, config):
        ...
        if config.lr_decay:
            self.lr_scheduler = torch.optim.lr_scheduler.StepLR(self.optimizer, step_size=config.lr_decay_step, gamma=config.lr_decay_factor,
                                                                last_epoch=config.start_epoch-1)
        else:
            self.lr_scheduler = None

    def predict_(self, batch):
        return self.forward(*self.get_inputs(batch, with_label=False))
